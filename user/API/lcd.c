...
void LCD_GPIOConfig(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOG,ENABLE);//使能PORTB,D,E,G以及AFIO复用功能时钟
 	...
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0;				 			//PB0 通用推挽输出 背光
...
//FSMC配置   关键参数FSMC_AccessMode_A   FSMC_Bank1_NORSRAM4
//参考官方代码    SRAM的例程
/*
FSMC外拓SRAM的时序与8080时序类似,所以用外拓SRAM的时序,模拟8080时序
*/
...
void LCD_GPIOConfig(uint8_t IO_Sate)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4|GPIO_Pin_5;				 //	通用推挽输出  
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //通用推挽输出   
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOD, &GPIO_InitStructure); 	
	if(IO_Sate==0)
...
{
    GPIO_WriteBit(D0_GPIO_Port,D0_Pin,(BitAction)(x>>0&0x0001));
    GPIO_WriteBit(D1_GPIO_Port,D1_Pin,(BitAction)(x>>1&0x0001));
    GPIO_WriteBit(D2_GPIO_Port,D2_Pin,(BitAction)(x>>2&0x0001));
    GPIO_WriteBit(D3_GPIO_Port,D3_Pin,(BitAction)(x>>3&0x0001));
    GPIO_WriteBit(D4_GPIO_Port,D4_Pin,(BitAction)(x>>4&0x0001));
    GPIO_WriteBit(D5_GPIO_Port,D5_Pin,(BitAction)(x>>5&0x0001));
    GPIO_WriteBit(D6_GPIO_Port,D6_Pin,(BitAction)(x>>6&0x0001));
    GPIO_WriteBit(D7_GPIO_Port,D7_Pin,(BitAction)(x>>7&0x0001));
    GPIO_WriteBit(D8_GPIO_Port,D8_Pin,(BitAction)(x>>8&0x0001));
    GPIO_WriteBit(D9_GPIO_Port,D9_Pin,(BitAction)(x>>9&0x0001));
    GPIO_WriteBit(D10_GPIO_Port,D10_Pin,(BitAction)(x>>10&0x0001));
    GPIO_WriteBit(D11_GPIO_Port,D11_Pin,(BitAction)(x>>11&0x0001));
    GPIO_WriteBit(D12_GPIO_Port,D12_Pin,(BitAction)(x>>12&0x0001));
    GPIO_WriteBit(D13_GPIO_Port,D13_Pin,(BitAction)(x>>13&0x0001));
    GPIO_WriteBit(D14_GPIO_Port,D14_Pin,(BitAction)(x>>14&0x0001));
    GPIO_WriteBit(D15_GPIO_Port,D15_Pin,(BitAction)(x>>15&0x0001));
}
uint16_t DATAIN()
{
    uint16_t data = 0;
    data |= GPIO_ReadInputDataBit(D15_GPIO_Port,D15_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D14_GPIO_Port,D14_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D13_GPIO_Port,D13_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D12_GPIO_Port,D12_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D11_GPIO_Port,D11_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D10_GPIO_Port,D10_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D9_GPIO_Port,D9_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D8_GPIO_Port,D8_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D7_GPIO_Port,D7_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D6_GPIO_Port,D6_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D5_GPIO_Port,D5_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D4_GPIO_Port,D4_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D3_GPIO_Port,D3_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D2_GPIO_Port,D2_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D1_GPIO_Port,D1_Pin);data <<= 1;
    data |= GPIO_ReadInputDataBit(D0_GPIO_Port,D0_Pin);
    return data;
}
